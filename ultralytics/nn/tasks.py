...
class BaseModel(torch.nn.Module):
    ...
    def loss(self, batch, preds=None):
        ...
        self.criterion.current_epoch = getattr(self, "current_epoch", 0)
        self.criterion.total_epochs = getattr(self, "total_epochs", None)
...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        loss_fn = CBRDetectLoss if isinstance(self.model[-1], CBRDetect) else v8DetectionLoss
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is C2fCIB:
                legacy = False
        elif m in {LiteAlignFuse, SCBAlignFuse, FAAFusion, ContextGuideFuse, SelectiveScaleFuse, RMSBalanceFuse}:
            c2 = args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            ...
            args = [ch[f]]
        elif m in {Concat, ShuffleConcat}:
            ...
            if m in {Detect, DetectECA, CBRDetect, C2fDetect, CoordAttnDetect, DilatedDetect, GDDetect, GDDetectV2, MCADetect, ShareConvDetect, TaskAlignDetect, TripletDetect, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26}:
                m.legacy = legacy
        elif m is v10Detect:
            args.extend([reg_max, [ch[x] for x in f]])
