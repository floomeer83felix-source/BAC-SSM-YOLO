...
from .block import C2f, DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Proto26, RealNVP, Residual, SwiGLUFFN
...
class DetectECA(Detect):
    """YOLO Detect head with ECA (Efficient Channel Attention) in cls/reg branches.

    Inserts lightweight ECA after the first conv block in both the regression (cv2) and
    classification (cv3) branches to improve representational power at minimal cost.
    """

    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        super().__init__(nc, reg_max, end2end=False, ch=ch)  # build parent first, then override
        from .block import ECA

        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))

        # Regression branch: Conv → ECA → Conv → Conv2d
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), ECA(c2), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1))
            for x in ch
        )

        # Classification branch with ECA after first DWConv+Conv block
        self.cv3 = (
            nn.ModuleList(
                nn.Sequential(Conv(x, c3, 3), ECA(c3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1))
                for x in ch
            )
            if self.legacy
            else nn.ModuleList(
...
class ShareConvDetect(Detect):
    """Detection head with shared first conv between cls/reg branches (OptiConv concept)."""
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        self.shared_conv = nn.ModuleList(Conv(x, c_shared, 3) for x in ch)
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(c_shared, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(c_shared, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
            else nn.ModuleList(
                nn.Sequential(
                    nn.Sequential(DWConv(c_shared, c_shared, 3), Conv(c_shared, c3, 1)),
                    nn.Conv2d(c3, self.nc, 1),
                )
                for x in ch
            )
    ...
    def forward_head(self, x, box_head=None, cls_head=None):
        if box_head is None or cls_head is None:
            return dict()
        bs = x[0].shape[0]
        shared = [self.shared_conv[i](x[i]) for i in range(self.nl)]
        boxes = torch.cat([box_head[i](shared[i]).view(bs, 4 * self.reg_max, -1) for i in range(self.nl)], dim=-1)
        scores = torch.cat([cls_head[i](shared[i]).view(bs, self.nc, -1) for i in range(self.nl)], dim=-1)
        return dict(boxes=boxes, scores=scores, feats=x)
...
class TaskAlignDetect(Detect):
    """Task-aligned detection head with cls/reg feature interaction (TOOD/MTD-YOLO concept)."""

    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        super().__init__(nc, reg_max, end2end=False, ch=ch)
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))

        self.cv2_inter = nn.ModuleList(Conv(x, c2, 3) for x in ch)
        self.cv3_inter = nn.ModuleList(
            nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)) if not self.legacy else nn.Sequential(Conv(x, c3, 3))
            for x in ch
        )
        self.cls2reg = nn.ModuleList(Conv(c3, c2, 1) for x in ch)
        self.reg2cls = nn.ModuleList(Conv(c2, c3, 1) for x in ch)

        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
            else nn.ModuleList(
                nn.Sequential(
                    nn.Sequential(DWConv(c3, c3, 3), Conv(c3, c3, 1)),
    ...
    def forward_head(self, x, box_head=None, cls_head=None):
        ...
        for i in range(self.nl):
            reg_feat = self.cv2_inter[i](x[i])
            cls_feat = self.cv3_inter[i](x[i])
            reg_feat_a = reg_feat * self.cls2reg[i](cls_feat).sigmoid()
            cls_feat_a = cls_feat * self.reg2cls[i](reg_feat).sigmoid()
            boxes.append(box_head[i](reg_feat_a).view(bs, 4 * self.reg_max, -1))
            scores.append(cls_head[i](cls_feat_a).view(bs, self.nc, -1))
...
class DilatedDetect(Detect):
    """Detection head with dilated convolutions (d=2) for larger effective receptive field."""
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3, d=2), Conv(c2, c2, 3, d=2), nn.Conv2d(c2, 4 * self.reg_max, 1))
            for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(x, c3, 3, d=2), Conv(c3, c3, 3, d=2), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
            else nn.ModuleList(
                nn.Sequential(
                    nn.Sequential(DWConv(x, x, 3, d=2), Conv(x, c3, 1)),
                    nn.Sequential(DWConv(c3, c3, 3, d=2), Conv(c3, c3, 1)),
                    nn.Conv2d(c3, self.nc, 1),
                )
                for x in ch
            )
...
class GDDetect(Detect):
    """Detection head with cross-scale Gather-Distribute feature fusion (Gold-YOLO concept).

    After extracting per-scale features, fuses them via interpolation-based cross-scale interaction
    before the final prediction layers.
    """
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        self.inter_cv2 = nn.ModuleList(Conv(x, c2, 3) for x in ch)
        self.inter_cv3 = nn.ModuleList(
            nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)) if not self.legacy else nn.Sequential(Conv(x, c3, 3))
            for x in ch
        )
        self.cv2 = nn.ModuleList(
    ...
    @staticmethod
    def _cross_fuse(feats: list[torch.Tensor]) -> list[torch.Tensor]:
        """Cross-scale feature fusion: neighboring scales exchange info via interpolation."""
        if len(feats) == 3:
            out = list(feats)
            out[0] = feats[0] + F.interpolate(feats[1], size=feats[0].shape[2:], mode="bilinear")
            out[2] = feats[2] + F.interpolate(feats[1], size=feats[2].shape[2:], mode="bilinear")
            mid = feats[1]
            s_up = F.interpolate(feats[0], size=mid.shape[2:], mode="bilinear")
            s_down = F.interpolate(feats[2], size=mid.shape[2:], mode="bilinear")
            out[1] = mid + s_up + s_down
            return out
        return feats
    ...
    def forward_head(self, x, box_head=None, cls_head=None):
        if box_head is None or cls_head is None:
            return dict()
        bs = x[0].shape[0]
        reg_feats = [self.inter_cv2[i](x[i]) for i in range(self.nl)]
        cls_feats = [self.inter_cv3[i](x[i]) for i in range(self.nl)]
        reg_feats = self._cross_fuse(reg_feats)
        cls_feats = self._cross_fuse(cls_feats)
        boxes = torch.cat([box_head[i](reg_feats[i]).view(bs, 4 * self.reg_max, -1) for i in range(self.nl)], dim=-1)
        scores = torch.cat([cls_head[i](cls_feats[i]).view(bs, self.nc, -1) for i in range(self.nl)], dim=-1)
        return dict(boxes=boxes, scores=scores, feats=x)
...
class GDDetectV2(Detect):
    """Enhanced GDDetect with BiFPN-style learnable weighted fusion + channel modulation.

    Improves upon GDDetect by:
    1. Learnable per-connection fusion weights (ReLU-normalized, BiFPN-style)
    2. Per-level channel-wise affine modulation after fusion
    3. Residual connection preserving original per-scale features
    """
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        # Per-scale feature extraction (same as GDDetect)
        self.inter_cv2 = nn.ModuleList(Conv(x, c2, 3) for x in ch)
        self.inter_cv3 = nn.ModuleList(
            nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)) if not self.legacy else nn.Sequential(Conv(x, c3, 3))
            for x in ch
        ...
        # Learnable fusion weights: 4 cross-connections in 3-scale PANet
        # P3←P4, P5←P4, P4←P3, P4←P5
        self.fuse_w_reg = nn.Parameter(torch.ones(4))
        self.fuse_w_cls = nn.Parameter(torch.ones(4))

        # Per-level channel modulation (affine: scale + bias) after fusion
        self.fuse_mod_reg = nn.Parameter(torch.ones(3, c2, 1, 1))   # per-level scale
        self.fuse_bias_reg = nn.Parameter(torch.zeros(3, c2, 1, 1)) # per-level bias
        self.fuse_mod_cls = nn.Parameter(torch.ones(3, c3, 1, 1))
        self.fuse_bias_cls = nn.Parameter(torch.zeros(3, c3, 1, 1))

        # Prediction tails (same as baseline)
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
    ...
    @staticmethod
    def _weighted_fusion(feats: list[torch.Tensor], w: torch.Tensor) -> list[torch.Tensor]:
        """BiFPN-style weighted fusion with softmax normalization over positive weights."""
        if len(feats) != 3:
            return feats
        eps = 1e-4
        w = F.relu(w) + eps
        w = w / w.sum()
        out = list(feats)
        out[0] = feats[0] + w[0] * F.interpolate(feats[1], size=feats[0].shape[2:], mode="bilinear")
        out[2] = feats[2] + w[1] * F.interpolate(feats[1], size=feats[2].shape[2:], mode="bilinear")
        mid = feats[1]
        s_up = F.interpolate(feats[0], size=mid.shape[2:], mode="bilinear")
        s_down = F.interpolate(feats[2], size=mid.shape[2:], mode="bilinear")
        out[1] = mid + w[2] * s_up + w[3] * s_down
        return out

    def _modulate(self, feats: list[torch.Tensor], scale: torch.Tensor, bias: torch.Tensor) -> list[torch.Tensor]:
        """Apply per-level channel-wise affine modulation (scale + bias)."""
        return [feats[i] * scale[i] + bias[i] for i in range(len(feats))]

    def forward_head(self, x, box_head=None, cls_head=None):
        if box_head is None or cls_head is None: return dict()
        bs = x[0].shape[0]
        reg_feats = [self.inter_cv2[i](x[i]) for i in range(self.nl)]
        cls_feats = [self.inter_cv3[i](x[i]) for i in range(self.nl)]

        # Fuse -> modulate -> residual
        reg_fused = self._weighted_fusion(reg_feats, self.fuse_w_reg)
        reg_fused = self._modulate(reg_fused, self.fuse_mod_reg, self.fuse_bias_reg)
        reg_feats = [reg_feats[i] + reg_fused[i] for i in range(self.nl)]

        cls_fused = self._weighted_fusion(cls_feats, self.fuse_w_cls)
        cls_fused = self._modulate(cls_fused, self.fuse_mod_cls, self.fuse_bias_cls)
        cls_feats = [cls_feats[i] + cls_fused[i] for i in range(self.nl)]
...
class MCABlock(nn.Module):
    """Multi-scale Context Aggregation block with ASPP-style dilated conv branches."""

    def __init__(self, c1: int, c2: int, dilations: tuple = (1, 2, 3), reduction: int = 4):
        """
        Initialize MCA block.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            dilations (tuple): Dilation rates for parallel branches.
            reduction (int): Channel reduction ratio for the shared compressed representation.
        """
        super().__init__()
        c_ = max(c2 // reduction, 4)
        self.compress = Conv(c1, c_, 1)
        self.branches = nn.ModuleList(
            nn.Sequential(Conv(c_, c_, 3, d=d, g=c_), Conv(c_, c_, 1)) for d in dilations
        )
        self.expand = Conv(c_ * len(dilations), c2, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass: compress -> multi-scale depthwise separable convs -> expand."""
        x = self.compress(x)
        x = torch.cat([b(x) for b in self.branches], dim=1)
        return self.expand(x)


class C2fDetect(Detect):
    """Detection head with C2f blocks replacing the second conv in each branch for deeper feature transformation."""
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), C2f(c2, c2, n=1, shortcut=False), nn.Conv2d(c2, 4 * self.reg_max, 1))
            for x in ch
        )
        self.cv3 = (
            nn.ModuleList(
            ...
            if not self.legacy
                ...
                nn.Sequential(Conv(x, c3, 3), C2f(c3, c3, n=1, shortcut=False), nn.Conv2d(c3, self.nc, 1))
                for x in ch
            )
        )
        self._end2end = False
...
class MCADetect(Detect):
    """Detection head with Multi-scale Context Aggregation (MCA) blocks before the final prediction convs."""
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), MCABlock(c2, c2), nn.Conv2d(c2, 4 * self.reg_max, 1))
            for x in ch
        )
        self.cv3 = (
            nn.ModuleList(
            ...
            if not self.legacy
                ...
                nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), MCABlock(c3, c3), nn.Conv2d(c3, self.nc, 1))
                for x in ch
            )
        )
        self._end2end = False
...
class TripletDetect(Detect):
    """Detection head with 3 parallel branches (standard, lightweight, dilated) merged via addition."""

    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        super().__init__(nc, reg_max, end2end=False, ch=ch)
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))

        self.cv2_p1 = nn.ModuleList(Conv(x, c2, 3) for x in ch)
        self.cv2_p2 = nn.ModuleList(Conv(x, c2, 1) for x in ch)
        self.cv2_p3 = nn.ModuleList(Conv(x, c2, 3, d=2) for x in ch)
        self.cv2_final = nn.ModuleList(nn.Conv2d(c2, 4 * self.reg_max, 1) for _ in ch)

        if self.legacy:
            self.cv3_p1 = nn.ModuleList(Conv(x, c3, 3) for x in ch)
            self.cv3_p2 = nn.ModuleList(Conv(x, c3, 1) for x in ch)
            self.cv3_p3 = nn.ModuleList(Conv(x, c3, 3, d=2) for x in ch)
        else:
            self.cv3_p1 = nn.ModuleList(nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)) for x in ch)
            self.cv3_p2 = nn.ModuleList(nn.Sequential(DWConv(x, x, 1), Conv(x, c3, 1)) for x in ch)
            self.cv3_p3 = nn.ModuleList(nn.Sequential(DWConv(x, x, 3, d=2), Conv(x, c3, 1)) for x in ch)
        self.cv3_final = nn.ModuleList(nn.Conv2d(c3, self.nc, 1) for _ in ch)

        self._end2end = False
        if end2end:
            self._end2end = True
            self.one2one_cv2 = copy.deepcopy(self.cv2_final)
            self.one2one_cv3 = copy.deepcopy(self.cv3_final)
    ...
    def forward_head(self, x, box_head=None, cls_head=None):
        ...
        boxes, scores = [], []
        for i in range(self.nl):
            boxes.append(box_head[i]((self.cv2_p1[i](x[i]) + self.cv2_p2[i](x[i]) + self.cv2_p3[i](x[i])) / 3).view(bs, 4 * self.reg_max, -1))
            scores.append(cls_head[i]((self.cv3_p1[i](x[i]) + self.cv3_p2[i](x[i]) + self.cv3_p3[i](x[i])) / 3).view(bs, self.nc, -1))
        return dict(boxes=torch.cat(boxes, -1), scores=torch.cat(scores, -1), feats=x)

    @property
    def one2many(self):
        return dict(box_head=self.cv2_final, cls_head=self.cv3_final)
    ...
    def bias_init(self):
        """Initialize biases for the three-branch detection head."""
        for i, (a, b) in enumerate(zip(self.cv2_final, self.cv3_final)):
            a.bias.data[:] = 2.0
            b.bias.data[: self.nc] = math.log(5 / self.nc / (640 / self.stride[i]) ** 2)
        if self._end2end:
...
class CoordAttnDetect(Detect):
    """Detection head with Coordinate Attention (CA) in cls/reg branches."""

    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        super().__init__(nc, reg_max, end2end=False, ch=ch)
        from .block import CoordAttn

        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))

        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), CoordAttn(c2), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1))
            for x in ch
        )
        self.cv3 = (
            nn.ModuleList(
            ...
            if not self.legacy
                ...
                nn.Sequential(Conv(x, c3, 3), CoordAttn(c3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1))
                for x in ch
            )
        )
        self._end2end = False
...
class CBRDetect(Detect):
    """Classroom behavior re-decoupled detect head with training-only auxiliary outputs."""

    def _forward_cls_branch(self, branch: nn.Module, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Run a classification branch and expose the pre-logit feature map for training-time behavior supervision."""
        if isinstance(branch, nn.Sequential) and len(branch) > 1 and isinstance(branch[-1], nn.Conv2d):
            feat = x
            for layer in branch[:-1]:
                feat = layer(feat)
            return branch[-1](feat), feat
        scores = branch(x)
    ...
    def forward_head(
        ...
        """Return standard detection outputs and optional training-only behavior cues without adding inference params."""
        if box_head is None or cls_head is None:  # for fused inference
            return dict()

        bs = x[0].shape[0]
        box_preds, score_preds = [], []
        behavior_feats, level_scores = [], []
        for i in range(self.nl):
            box_preds.append(box_head[i](x[i]).view(bs, 4 * self.reg_max, -1))
            if return_behavior:
                score_map, feat_map = self._forward_cls_branch(cls_head[i], x[i])
                behavior_feats.append(feat_map.flatten(2))
                level_scores.append(score_map.view(bs, self.nc, -1))
            else:
                score_map = cls_head[i](x[i])
            score_preds.append(score_map.view(bs, self.nc, -1))

        preds = dict(boxes=torch.cat(box_preds, dim=-1), scores=torch.cat(score_preds, dim=-1), feats=x)
        if return_behavior:
            preds["behavior_feats"] = behavior_feats
            preds["level_scores"] = level_scores
        return preds

    def forward(
        self, x: list[torch.Tensor]
    ) -> dict[str, torch.Tensor] | torch.Tensor | tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """Run standard detection and attach auxiliary behavior cues only during training."""
        preds = self.forward_head(x, **self.one2many, return_behavior=self.training)
        if self.end2end:
            x_detach = [xi.detach() for xi in x]
            one2one = self.forward_head(x_detach, **self.one2one)
            preds = {"one2many": preds, "one2one": one2one}
...
class v10Detect(Detect):
    ...
    def __init__(self, nc: int = 80, reg_max: int = 16, ch: tuple = ()):
        ...
        Args:
            ...
            ch (tuple): Tuple of channel sizes from backbone feature maps.
        """
        super().__init__(nc, reg_max=reg_max, end2end=True, ch=ch)
