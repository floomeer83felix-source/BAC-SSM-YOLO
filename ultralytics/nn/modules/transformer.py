...
_LLSA_L1 = None
_LLSA_L2 = None
_LLSA_IMPORT_TRIED = False


def _get_llsa_ops():
    """Load official LLSA kernels when the external repository is available."""
    global _LLSA_IMPORT_TRIED, _LLSA_L1, _LLSA_L2
    if _LLSA_IMPORT_TRIED:
        return _LLSA_L1, _LLSA_L2
    _LLSA_IMPORT_TRIED = True

    root = Path(__file__).resolve().parents[4]
    external_src = root / "external" / "LLSA" / "src"
    ...
    try:
        ...
        _LLSA_L1, _LLSA_L2 = llsa_l1_varlen, llsa_l2_varlen
    except Exception:
        _LLSA_L1, _LLSA_L2 = None, None
    return _LLSA_L1, _LLSA_L2
...
class LLSABlock(nn.Module):
    """YOLO feature-map adapter for official Log-linear Sparse Attention (LLSA)."""
    ...
    def __init__(
        ...
        """Initialize an image-feature attention block backed by LLSA when available."""
        super().__init__()
        self.cv1 = Conv(c1, c2, 1, 1) if c1 != c2 else nn.Identity()
        self.pos = nn.Conv2d(c2, c2, 3, padding=1, groups=c2, bias=False)

        num_heads = min(num_heads, c2)
        while c2 % num_heads != 0 and num_heads > 1:
            num_heads -= 1
        self.num_heads = max(num_heads, 1)
        self.head_dim = c2 // self.num_heads
        self.block_size = int(block_size)
        self.topk1 = int(topk1)
        self.topk2 = int(topk2)

        self.norm = nn.LayerNorm(c2)
        self.qkv = nn.Linear(c2, c2 * 3, bias=False)
        self.proj = nn.Linear(c2, c2, bias=False)

        hidden = max(int(c2 * mlp_ratio), c2)
        self.ffn = nn.Sequential(Conv(c2, hidden, 1), Conv(hidden, c2, 1, act=False))
        self._disable_llsa = False

    def _attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        """Run official LLSA kernels when possible, otherwise fall back to SDPA."""
        if q.is_cuda and not self._disable_llsa:
            llsa_l1, llsa_l2 = _get_llsa_ops()
            try:
                if llsa_l1 is not None and q.shape[2] < 16384:
                    return llsa_l1(q, k, v, topk=self.topk1, block_size=self.block_size)
                if llsa_l2 is not None:
                    return llsa_l2(q, k, v, topk1=self.topk1, topk2=self.topk2, block_size=self.block_size)
            except Exception:
                self._disable_llsa = True
        return F.scaled_dot_product_attention(q, k, v, dropout_p=0.0, is_causal=False)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply context attention over flattened HxW tokens and restore the feature map."""
        x = self.cv1(x)
        x = x + self.pos(x)
        b, c, h, w = x.shape

        tokens = x.flatten(2).transpose(1, 2)
        qkv = self.qkv(self.norm(tokens)).reshape(b, h * w, 3, self.num_heads, self.head_dim)
        q, k, v = qkv.unbind(2)
        ...
        y = tokens.transpose(1, 2).reshape(b, c, h, w).contiguous()
        return y + self.ffn(y)
