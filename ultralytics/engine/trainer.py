...
class BaseTrainer:
            ...
            for i, batch in pbar:
                ...
                try:
                    with autocast(self.amp):
                        ...
                        loss, self.loss_items = self.compute_loss(batch)
                        self.loss = loss.sum()
                        if RANK != -1:
    ...
    def compute_loss(self, batch: dict):
        """Compute the training loss for a batch. Subclasses can override to add custom training logic."""
        model = unwrap_model(self.model)
        model.current_epoch = getattr(self, "epoch", 0)
        model.total_epochs = self.epochs
        if self.args.compile:
            preds = self.model(batch["img"])
            return model.loss(batch, preds)
